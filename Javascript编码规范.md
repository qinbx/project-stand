...
##函数声明##
函数应当在使用前提前定义。非方法函数应当使用函数定义格式（不是函数表达式和Function构造器格式）。函数名和开始圆括号间不应该有空格；结束的圆括号和花括号之间应该留一个空格；右侧的花括号应当同function关键字保持同一行；参数名之间应当在逗号之后保留一个空格。函数体应当保持一级缩进。

    // 好的写法
    function doSomething(arg1, arg2){
        return arg1 + args;
    }
其他函数定义的函数应当在var语句后立即定义。
...
匿名函数可能作为方法赋值给对象，或者作为其它函数的参数。function关键字同开始括号间不应有空格。
